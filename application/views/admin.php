...
<form method="post" action="/admin/produs" enctype="multipart/form-data">
	<table border="0">
                ...
                <select  name="category" style="font-size:14px; color:#000; margin:10px 0px;">
                    ...
                    <?php foreach ($categories as $key => $category) { ?>
						...
						<?php if ($category['level'] == 1) { ?>
							...
							<?php } else { ?>
                                <option value="<?= $key ?>"><?= $category['title_ro'] ?></option>                          
                            <?php } ?>
                        <?php } elseif ($category['level'] == 2) { ?>
                        	<option value="<?= $key ?>" <?php if ($category['clickable'] == false) { ?> disabled="disabled"<?php } ?>><?= $category['title_ro'] ?></option>                          
    	...
    	<tr>
            ...
            <td width="100"></td>
            <td>
            	<input class="admin" type="text" name="code" />
            </td>
        </tr>
        <tr><td colspan="3" height="20"></td></tr>
    	<tr>
